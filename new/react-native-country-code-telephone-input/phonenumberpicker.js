...
import CallingCodeToCCA2 from "libphonenumber-js/metadata.min.json"
/* maps  CCA2 ->  callingCode      */
import CallingCodes from "./react-native-country-picker-modal/data/countries"

import CountryPicker from "./react-native-country-picker-modal/src/CountryPicker"
/* maps CCA2 -> CountryLocalDetails */
import Countries from "./country-data"
import { COUNTRY_NAMES } from "../../../../i18n/countries"
import RNLanguages from "react-native-languages"

const deviceLanguage = RNLanguages.language.slice(0, 2).toLowerCase()
const deviceCountry = DeviceInfo.getDeviceCountry().toLowerCase()

const getInitialCountry = () => {
  let name, callingCode
  const cca2 = DeviceInfo.getDeviceCountry()
  try {
    name = COUNTRY_NAMES[deviceLanguage].countries[deviceCountry]
    callingCode = CallingCodes[cca2].callingCode
  } catch (error) {
    console.warn(
      "PhoneNumberPicker.tsx - getInitialCountry - country translation doesn't exist: fallback to English",
    )
    console.tron.log(
      "PhoneNumberPicker.tsx - getInitialCountry - country translation doesn't exist: fallback to English",
    )
...
class PhoneNumberPicker extends React.Component {
  ...
  getCountryFromCCA2(cca2) {
      ...
      for (let i = 0; i < Countries.length; i++) {
        if (Countries[i].code.toUpperCase() == cca2.toUpperCase()) {
          try {
            ...
            console.warn(
              "PhoneNumberPicker.tsx - getCountryFromCCA2 - country translation doesn't exist: fallback to English",
            )
            console.tron.log(
              "PhoneNumberPicker.tsx - getCountryFromCCA2 - country translation doesn't exist: fallback to English",
            )
    ...
    if (countryName.length == 0 || cca2.length == 0 || callingCode.length == 0) {
      countryName =
        this.state.country.callingCode.length > 0 ? "Invalid country code" : "Choose a country"
  ...
  FlagPickedChanged(updatedCountry) {
    ...
    this.setState({ country: updatedCountry, cca2 })
  ...
  SafeRenderCountryPicker(cca2) {
    ...
    //   <Image style={{ width: 20, height: 20 }} source={require("./down.png")} />
    //     </TouchableOpacity>
    //   </View>
    // )
    console.tron.display({
      name: cca2,
      value: this.state,
      preview: this.state.cca2,
      important: true,
    })
    console.tron.display({
      name: "countryFromCCA2",
      value: countryFromCCA2,
      preview: countryFromCCA2.cca2,
      important: true,
    })
    countryPicker = (
      <View style={[styles.containerRow, styles.viewBottomBorder]}>
