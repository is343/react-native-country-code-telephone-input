...
class PinPicker extends React.Component {
  ...
  renderPinInput() {
    ...
    for (let i = 0; i < this.props.numDigits; i++) {
      let onChangeText = text => {
        ...
        digits[i] = text
        this.setState({ digits: digits, setFocusIdx: setFocusIdx }, this.pinChanged.bind(this))
      }
      ...
      let onKeyPress = () => {
        ...
        let focusIdx = i
        if (this.state.digits[i] === undefined || this.state.digits[i].length == 0) {
  ...
  render() {
    return (
      <View style={[styles.containerRow, { marginHorizontal: 0 }]}>{this.renderPinInput()}</View>
    )
  }
}
