...
import BorderTextInput from "../../../shared/BorderTextInput"
...
interface PhoneNumberPickerProps {
  onChange: (callingCode: string, phoneNumber: string, cca2: string) => void
}
...
class PhoneNumberPicker extends React.Component<PhoneNumberPickerProps, PhoneNumberPickerState> {
  ...
  numberChanged = (callingCode: string, phoneNumber: string): void => {
    callingCode = callingCode + ""
    phoneNumber = phoneNumber + ""
    callingCode = callingCode.replace(/\D/g, "")
    phoneNumber = phoneNumber.replace(/\D/g, "")
    const { cca2 } = this.state.country
    this.state.onChange(callingCode, phoneNumber, cca2)
  }
  ...
  getCountryFromCCA2(cca2: string): Country {
    ...
    if (countryName !== this.state.previousToast) {
      Toast.showWithGravity(countryName, Toast.SHORT, Toast.TOP)
      this.setState({ previousToast: countryName })
    }
    return {
      name: countryName,
      cca2: cca2,
  ...
  FlagPickedChanged = (updatedCountry: Country): void => {
    ...
    setTimeout(() => {
      Toast.showWithGravity(updatedCountry.name, Toast.SHORT, Toast.TOP)
    }, 300)
    ...
    this.numberChanged(updatedCountry.callingCode, this.state.phoneNo)
  }
  ...
  CallingCodeChanged = (updatedCallingCode: string) => {
    ...
    if (
      ...
      Toast.showWithGravity(countryFromCallingCode.name, Toast.SHORT, Toast.TOP)
    }
    this.setState({
    ...
    this.numberChanged(countryFromCallingCode.callingCode, this.state.phoneNo)
  }
  ...
  PhoneChanged = updatedPhoneNo => {
    ...
    this.numberChanged(this.state.country.callingCode, updatedPhoneNo)
  }

  SafeRenderCountryPicker = (cca2: string): JSX.Element => {
    /*
      ...
      <View style={[styles.containerRow]}>
        ...
        >
          <CountryPicker
            ...
            closeable
            onChange={this.FlagPickedChanged}
  ...
  render() {
    return (
      <View style={styles.containerCol}>
        <BorderTextInput
          onChangeText={this.CallingCodeChanged}
          ref={ref => (this.countryCodeTextInput = ref)}
          value={"+" + this.state.callingCode}
          label={translate("welcome.countryCode")}
          leftComponent={this.SafeRenderCountryPicker(this.state.country.cca2)}
          keyboardType={"phone-pad"}
        />
        <BorderTextInput
          onChangeText={this.PhoneChanged}
          ref={ref => (this.textInputPhoneNumber = ref)}
          value={this.PhoneNumberFormatAsYouType()}
          label={translate("welcome.phoneNumber")}
          text={translate("welcome.smsWillSend")}
          keyboardType={"phone-pad"}
          focussed
        />
      </View>
